...
$this->Paginator->options(
        array('update' => '.box',
            ...
            'url' => array('controller' => 'services',
                'action' => 'listService', 'admin' => true, 'text' => (!empty($text)) ? $text : '',),
));
?> 
...
<table id="demo-foo-addrow" class="table m-t-30 table-hover contact-list footable-loaded footable" data-page-size="10">
    ...
    <tbody>
        <?php
        if ($services):
             $page = $this->request->params['paging']['Service']['page'];/*page number exe. page no.2 */
             $record_limit = $this->request->params['paging']['Service']['limit'];/* limit 10*/
            ...
            foreach ($services as $category):
                ...
                <td><?php echo $count; ?></td>
                 <td>
                     <?php
                    $m = FULL_BASE_URL . $this->webroot . $category['Service']['image'];
                    $default = FULL_BASE_URL . $this->webroot . 'img/users/default.png';

                    if (!empty($category['Service']['image'])) {
                        $img = $this->Html->image($m, array('alt' => 'Service Image', 'border' => '1',
                            'height' => '40', 'width' => '40',
                ...
                ?>
                   ...
                   <td><?= $category['Service']['name_en'];?></td>
                    <td><?= $category['Service']['name_ar'];?></td>
                    <td><?= $category['Service']['name_ku'];?></td>
                    <td>
                <?php echo
                $this->Html->link(
                ...
                $category['Service']['id'], 'listService',
                ), array('escape' => false)
                );
                ?>  
                <button type="button" title='Delete' class="cstm-des-rounded-sec btn btn-danger delete_user model_img" u_id ="<?= $category['Service']['id'] ?>" name="button" >
       ...
       var text = $('.myInput').val();
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    type: "POST",
                    url: root + 'admin/services/deleteService/text:'+ text 
                        + '/page:<?= !empty($this->passedArgs['page']) ? $this->passedArgs['page'] : '1' ?>' 
                        + '/service:<?= !empty($this->passedArgs['sort']) ? $this->passedArgs['sort'] : 'Service.id' ?>' 
