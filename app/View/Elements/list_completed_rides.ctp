...
$this->Paginator->options(
        array('update' => '.box',
            ...
            'url' => array('controller' => 'rides',
                'action' => 'listCompletedRide', 'admin' => true, 'text' => (!empty($text)) ? $text : '',),
));
?>
            ...
            if ($rides):
                foreach ($rides as $ride):
                    ...
                    <tr>
                        ...
                        <td> <?php echo  $rstatus = $ride_status[$ride['Ride']['status']];?></td>
                        ...
                        <td>
                            <button type="button" title="View" data-toggle='tooltip' title='View' class="cstm-des-rounded-sec btn btn-danger" u_id ="<?= $ride['Ride']['id'] ?>" name="button">
                                <?=
                            ...
                            <button type="button" title="Delete" data-toggle='tooltip' title='Delete' class="cstm-des-rounded-sec btn btn-danger delete_ride" u_id ="<?= $ride['Ride']['id'] ?>" name="button"><i class="fa fa-times" aria-hidden="true"></i></button>
                        </td>
                    </tr>
                    <?php
                endforeach;
...
<script>
    ...
    $('.delete_ride').click(function () {
        ...
        if (confirm('Are you sure you want to delete this Ride?')) {
            ...
            $.ajax({
                type: "POST",
                url: root + 'admin/rides/deleteCompletedRide/text:' + text + '/page:<?= !empty($this->passedArgs['page']) ? $this->passedArgs['page'] : '1' ?>',
                data: {
                    id: $(this).attr('u_id'),
                },
