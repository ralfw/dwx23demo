...
<table id="demo-foo-addrow" class="table m-t-30 table-hover contact-list footable-loaded footable" data-page-size="10">
    ...
    <tbody>
        <?php
        if ($offices):
             $page = $this->request->params['paging']['Office']['page'];/*page number exe. page no.2 */
             $record_limit = $this->request->params['paging']['Office']['limit'];/* limit 10*/
             $count = (($page-1)*$record_limit)+1;
            foreach ($offices as $category):
                
                ?>
                <tr>
                <td><?php echo $count; ?></td>
                <td> <?=
                    $this->Html->link('' . __($category['Office']['location_en'] ). '', array(
                    'controller' => 'offices',
                    'action' => 'viewOffice',  
                    $category['Office']['id'],
                   ), array('escape' => false));
                ...
                <td><?= $category['Office']['address_en'];?></td>
                <td>
                
                <button type="button" title='Delete' class="cstm-des-rounded-sec btn btn-danger delete_user model_img" u_id ="<?= $category['Office']['id'] ?>" name="button" >
       ...
       var text = $('.myInput').val();
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    type: "POST",
                    url: root + 'admin/offices/deleteOffice/text:'+ text 
                        + '/page:<?= !empty($this->passedArgs['page']) ? $this->passedArgs['page'] : '1' ?>' 
                        + '/office:<?= !empty($this->passedArgs['sort']) ? $this->passedArgs['sort'] : 'Office.id' ?>' 
