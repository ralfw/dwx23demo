...
$this->Paginator->options(
        array('update' => '.box',
            ...
            'url' => array('controller' => 'users',
                'action' => 'listProvider', 'admin' => true, 'text' => (!empty($text)) ? $text : '',),
));
?> 
          ...
          <th>Action</th>
                ...
                <?php
                if ($users):
                $page = $this->request->params['paging']['User']['page'];/*page number exe. page no.2 */
                $record_limit = $this->request->params['paging']['User']['limit'];/* limit 10*/
                ...
                ?>
                 ...
                 <td>
                <?=
                $this->Html->link(''.__($user['User']['name']).'', array(
                'controller' => 'users',
                'action' => 'viewProvider',
                'page' => !empty($this->passedArgs['page']) ? $this->passedArgs['page'] : '1',
                $user['User']['id'], 'listProvider' ), array('escape' => false));
                ?>
       ...
       var text = $('.myInput').val();
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    type: "POST",
                    url: root + 'admin/users/deleteProvider/text:'+ text 
                        + '/page:<?= !empty($this->passedArgs['page']) ? $this->passedArgs['page'] : '1' ?>' 
...
//                }
  ...
  $('.change').click(function () {
       ...
       var text = $('.myInput').val();
        ...
        swal({
            title: "Are you sure?",
            text: "You want to deactivate the provider",
            type: "warning",
            timer: 3000,
            showCancelButton: true,
            confirmButtonClass: "btn-danger",
            ...
            if (isConfirm) {
                ...
                $.ajax({
                type: "POST",
                url: root + 'admin/users/changeproviderStatus/text:' + text
                        + '/page:<?= !empty($this->passedArgs['page']) ? $this->passedArgs['page'] : '1' ?>'
                        + '/sort:<?= !empty($this->passedArgs['sort']) ? $this->passedArgs['sort'] : 'User.id' ?>'
                        + '/direction:<?= !empty($this->passedArgs['direction']) ? $this->passedArgs['direction'] : 'DESC' ?>',
                data: {
