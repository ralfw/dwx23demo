...
$this->Paginator->options(
        array('update' => '.box',
            ...
            'url' => array('controller' => 'users',
                'action' => 'listSaloon', 'admin' => true, 'text' => (!empty($text)) ? $text : '',),
));
?> 
                                    ...
                                    <table id="demo-foo-addrow" class="table m-t-30 table-hover contact-list footable-loaded footable" data-page-size="10">
                                        ...
                                        ?>
                                            <tr class="footable-<?php echo $mine; ?>">
                                                ...
                                                <td>
                                                 <?=
                                                    $this->Html->link("<button type='button' data-toggle='tooltip' class='cstm-des-rounded-sec btn btn-info'><i class='fa fa-eye' aria-hidden='true'></i></button>", array(
                                                    'controller' => 'users',
                                                    'action' => 'viewSalon',
...
<script>
     $('.delete_user').click(function () {
       ...
       var text = $('.myInput').val();
        ...
        swal({
            title: "Are you sure?",
            text: "You want to delete this saloon !",
            type: "warning",
            timer: 3000,
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    type: "POST",
                    url: root + 'admin/users/deleteSaloon/text:'+ text 
       ...
       var id = $(this).attr('u_id');
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                type: "POST",
                url: root + 'admin/users/changeSaloonstatus/text:' + text
                        + '/page:<?= !empty($this->passedArgs['page']) ? $this->passedArgs['page'] : '1' ?>'
                        + '/sort:<?= !empty($this->passedArgs['sort']) ? $this->passedArgs['sort'] : 'User.id' ?>'
                        + '/direction:<?= !empty($this->passedArgs['direction']) ? $this->passedArgs['direction'] : 'DESC' ?>',
                data: {
